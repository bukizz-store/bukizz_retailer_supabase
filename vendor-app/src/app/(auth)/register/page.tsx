...
import { OnboardingStep, BusinessCategory } from '@/types';
...
export default function RegisterPage() {
    const [currentStep, setCurrentStep] = useState<OnboardingStep>('mobile');
    const [completedSteps, setCompletedSteps] = useState<OnboardingStep[]>([]);
    const [mobileVerified, setMobileVerified] = useState(false);
    const [emailVerified, setEmailVerified] = useState(false);
    const [otpSent, setOtpSent] = useState<'mobile' | 'email' | null>(null);
    const [businessCategory, setBusinessCategory] = useState<BusinessCategory>();

    // Form instances for each step
    const mobileForm = useForm<MobileVerificationData>({
    ...
    const handleBusinessSubmit = async (data: BusinessDetailsData) => {
        console.log('Business data:', data);
        setCompletedSteps(prev => [...prev, 'business']);
        setCurrentStep('bank');
    };

    const handleBankSubmit = async (data: BankDetailsData) => {
    ...
    const goBack = () => {
        const stepOrder: OnboardingStep[] = ['mobile', 'email', 'business', 'bank'];
