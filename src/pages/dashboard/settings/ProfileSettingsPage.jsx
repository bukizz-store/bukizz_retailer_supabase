            ...
            <div className="relative w-full max-w-lg rounded-2xl border border-slate-200 bg-white shadow-2xl animate-fade-in overflow-hidden">
                ...
                <div className="px-6 pt-5 pb-6">
                    ...
                    <div className="space-y-0 divide-y divide-slate-100">
                        ...
                        <InfoRow icon={Mail} label="Contact Email" value={warehouse.contact_email} copyable />
                        <InfoRow icon={Phone} label="Contact Phone" value={warehouse.contact_phone} copyable />
                    ...
                    {(fullAddress || addr.city || pincode) && (
                        <>
                            ...
                            <div className="divide-y divide-slate-100">
                                {fullAddress && <InfoRow icon={MapPin} label="Street Address" value={fullAddress} />}
                                {addr.city && <InfoRow icon={Building2} label="City" value={addr.city} />}
                                {addr.state && <InfoRow icon={MapPin} label="State" value={addr.state} />}
                                {pincode && <InfoRow icon={FileText} label="Pincode" value={pincode} mono />}
                                <InfoRow icon={Globe} label="Country" value={addr.country || 'India'} />
                            </div>
                        </>
                    )}
                </div>
...
const BankFormModal = ({ isOpen, onClose, onSave, editData }) => {
    ...
    const validate = () => {
        ...
        else if (!/^\d{9,18}$/.test(form.accountNumber.trim())) e.accountNumber = 'Enter a valid account number (9-18 digits)';
        ...
        else if (!/^[A-Z]{4}0[A-Z0-9]{6}$/.test(form.ifscCode.trim().toUpperCase())) e.ifscCode = 'Enter a valid IFSC code (e.g. SBIN0001234)';
                    ...
                    */}
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400">
                                {editData ? 'Update your bank details' : 'Enter your bank account information'}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <label className="flex items-center gap-3 cursor-pointer rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 hover:bg-slate-100 transition-colors">
                            ...
                            />
                            <div>
                                <p className="text-sm font-medium text-slate-700">Set as primary account</p>
                                <p className="text-xs text-slate-400">Payments will be credited to this account</p>
                            </div>
                        </label>
                    </div>
            ...
            <div className="relative w-full max-w-sm rounded-2xl border border-slate-200 bg-white shadow-2xl animate-fade-in overflow-hidden">
                ...
                <div className="px-6 pt-5 pb-6 text-center">
                    ...
                    <p className="mt-2 text-sm text-slate-500">
                        Are you sure you want to remove <strong className="text-slate-700">{bankName}</strong> account?
...
export default function ProfileSettingsPage() {
            ...
            try {
                ...
                const retailer =
                    dataRes.status === 'fulfilled'
                        ? dataRes.value?.data?.data || dataRes.value?.data
                        : null;
    ...
    const handleSetPrimary = async (id) => {
        ...
        try {
            ...
            alert(err?.response?.data?.message || 'Failed to set primary account. Please try again.');
        } finally {
            setBankSaving(false);
        }
    };
    ...
    const displayName = user?.full_name || user?.fullName || profile?.full_name || 'Retailer';
    ...
    const ownerName = retailer?.ownerName || retailer?.owner_name || profile?.owner_name || '';
    const gstin = retailer?.gstin || profile?.gstin || '';
    const pan = retailer?.pan || retailer?.panNumber || retailer?.pan_number || profile?.pan || '';
    const businessCategory = retailer?.businessCategory || retailer?.business_category || profile?.business_category || '';
    const signatureUrl = retailer?.signatureUrl || retailer?.signature_url || profile?.signature_url || '';

    const verificationStatus = retailer?.status || profile?.verification_status || user?.status || '';
    const isVerified = verificationStatus === 'authorized' || verificationStatus === 'verified';
                ...
                <div className="space-y-6">
                        ...
                        <div className="relative px-6 pb-6">
                            ...
                            <div className="pt-14 sm:flex sm:items-end sm:justify-between">
                                ...
                                <div className="mt-3 flex flex-wrap gap-2 sm:mt-0">
                                    ...
                                    {businessCategory && (
                                        <Badge variant="info" dot>{formatCategory(businessCategory)}</Badge>
                                    )}
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                        <SectionCard icon={User} title="Account Information">
                            ...
                            <InfoRow icon={Phone} label="Mobile Number" value={phone} copyable />
                            <InfoRow
                                icon={ShieldCheck}
                        ...
                        <SectionCard
                            ...
                            badge={businessCategory ? <Badge variant="info">{formatCategory(businessCategory)}</Badge> : null}
                        >
                            <InfoRow icon={User} label="Owner Name" value={ownerName} />
                            <InfoRow icon={FileText} label="GSTIN" value={gstin} mono copyable />
                            <InfoRow icon={CreditCard} label="PAN Number" value={pan} mono copyable />
                            {signatureUrl && (
                                <div className="py-3">
                                    <p className="text-xs font-medium uppercase tracking-wider text-slate-400 mb-2">E-Signature</p>
                                    <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 flex items-center justify-center">
                                        <img src={signatureUrl} alt="E-Signature" className="max-h-16 object-contain" />
                                    </div>
                                </div>
                            )}
                        </SectionCard>
                    ...
                    <SectionCard
                        ...
                        badge={!whLoading && warehouses.length > 0 ? <Badge variant="approved" dot>{warehouses.length} Active</Badge> : null}
                    >
                        {whLoading ? (
                            ...
                            <div className="py-10 text-center">
                                ...
                                <p className="text-sm font-medium text-slate-600">No warehouses yet</p>
                                <p className="mt-1 text-xs text-slate-400">Warehouses you set up will appear here.</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 gap-3 py-3 sm:grid-cols-2 lg:grid-cols-3">
                                {warehouses.map((wh) => {
                                    const addr = wh.address || {};
                                    const shortAddr = [addr.city, addr.state].filter(Boolean).join(', ');
                                    return (
                                        <button
                                            key={wh.id}
                                            type="button"
                                        ...
                                        >
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    <p className="text-sm font-semibold text-slate-800 truncate">{wh.name}</p>
                                                    {shortAddr && (
                                                        <p className="mt-0.5 flex items-center gap-1 text-xs text-slate-500">
                                                            <MapPin className="h-3 w-3 flex-shrink-0" />{shortAddr}
                                                        </p>
                                                    )}
                                                </div>
                                            ...
                                            <div className="mt-3 flex flex-wrap gap-1.5">
                                                {wh.contact_email && (
                                                    <span className="inline-flex items-center gap-1 rounded-md bg-slate-50 px-2 py-0.5 text-[11px] text-slate-500">
                                                        <Mail className="h-3 w-3" />{wh.contact_email}
                                                    </span>
                                                )}
                                                {wh.contact_phone && (
                                                    <span className="inline-flex items-center gap-1 rounded-md bg-slate-50 px-2 py-0.5 text-[11px] text-slate-500">
                                                        <Phone className="h-3 w-3" />{wh.contact_phone}
                                                    </span>
                                                )}
                                            </div>
            ...
            {activeTab === 'bank' && (
                <div className="space-y-6">
                    ...
                    <SectionCard
                        ...
                        badge={
                            bankAccounts.length > 0 ? (
                                <Badge variant="approved" dot>{bankAccounts.length} Saved</Badge>
                            ) : null
                        }
                        action={
                            <Button
                    ...
                    >
                        {bankLoading ? (
                            ...
                            <div className="flex items-center justify-center gap-2 py-14 text-sm text-slate-400">
                                <Loader2 className="h-5 w-5 animate-spin" /> Loading bank accounts…
                            </div>
                        ) : bankAccounts.length === 0 ? (
                            /* ── empty state ─────────────────────────────── */
                            <div className="py-14 text-center">
                                ...
                                <p className="mt-1 text-sm text-slate-400 max-w-xs mx-auto">
                                    Add your bank account details to receive payments for your orders.
                                </p>
                                <Button
                                    size="sm"
                                    className="mt-5"
                                    ...
                                    >
                                        ...
                                        <div className="flex items-start justify-between mt-1">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    {bank.branchName && (
                                                        <p className="text-xs text-slate-400">{bank.branchName}</p>
                                                    )}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-4 space-y-2">
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                <Badge variant="default">
                                                    {bank.accountType === 'current' ? 'Current' : 'Savings'}
                                                </Badge>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-4 flex items-center gap-2 border-t border-slate-100 pt-3">
                                            {!bank.isPrimary && (
                                                ...
                                                >
                                                    {bankSaving ? <Loader2 className="h-3 w-3 animate-spin" /> : <Star className="h-3 w-3" />} Set Primary
                                                </button>
                                            )}
                    ...
                    <div className="flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                        ...
                        <div>
                            <p className="text-sm font-semibold text-emerald-800">Securely Synced</p>
                            <p className="mt-0.5 text-xs text-emerald-700 leading-relaxed">
                                Your bank details are securely saved on our server. 
                                They will be used for processing payouts once verification is complete.
                            </p>
                        </div>
                    </div>
                </div>
