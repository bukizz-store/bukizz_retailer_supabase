...
import {
    Search, Play, Download, ChevronDown, Package,
    Lightbulb, Truck, RotateCcw,
} from 'lucide-react';
...
export default function ReturnsPage() {
    ...
    const filteredReturns = mockReturns.filter(ret => {
        ...
        if (activeTab === 'important') return matchesSearch && (ret.status === 'in_transit' || ret.status === 'initiated');
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <button className="flex items-center gap-2 rounded-full bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition-colors">
                        <Play className="h-4 w-4 fill-current" />Learn about New Returns Experience
                    </button>
                </div>
                <Input placeholder="Order or Return Id or tracking Id" value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)} icon={<Search className="h-4 w-4" />} className="w-80" />
            </div>

            <div className="flex items-center justify-between border-b border-slate-200">
                <div className="flex">
                    {[{ id: 'important', label: 'Important for Today' }, { id: 'all', label: 'All Returns' }].map(tab => (
                ...
                <div className="flex items-center gap-2 pb-3">
                    ...
                    <a href="#" className="text-sm font-medium text-blue-600 hover:underline">Show How ›</a>
                </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <button className="flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
                    Location <span className="text-slate-900">Delhi NCR</span><ChevronDown className="h-4 w-4" />
                </button>
                <div className="flex items-center gap-3">
                    <button className="text-sm font-medium text-blue-600 hover:underline flex items-center gap-2">
                        <Download className="h-4 w-4" />Download Proof of Delivery
                    </button>
                    <Button variant="outline"><Truck className="h-4 w-4" />Reaching you Today</Button>
                </div>
            </div>
            ...
            <div className="flex items-center justify-between">
                <div className="flex gap-2">
                    <button onClick={() => setCarrierFilter('non_large')}
                        className={cn("rounded-full border px-4 py-2 text-sm font-medium transition-colors",
                            carrierFilter === 'non_large' ? "border-blue-600 bg-blue-50 text-blue-700" : "border-slate-300 bg-white text-slate-700 hover:bg-slate-50"
                        )}>Standard Delivery ({mockReturns.filter(r => r.returnType === 'customer_return').length})</button>
                    <button onClick={() => setCarrierFilter('large')}
                        className={cn("rounded-full border px-4 py-2 text-sm font-medium transition-colors",
                            carrierFilter === 'large' ? "border-blue-600 bg-blue-50 text-blue-700" : "border-slate-300 bg-white text-slate-700 hover:bg-slate-50"
                        )}>Express Delivery ({mockReturns.filter(r => r.returnType === 'courier_return').length})</button>
                </div>
                <Button>Generate OTC</Button>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-slate-200 bg-slate-50">
                                ...
                                <th className="px-6 py-4 text-left text-sm font-semibold text-slate-900">Product Details</th>
                                <th className="px-6 py-4 text-left text-sm font-semibold text-slate-900">Return Type</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {filteredReturns.map((returnItem) => (
                                <tr key={returnItem.id} className="transition-colors hover:bg-slate-50">
                                    <td className="px-6 py-4">
                                        <p className="font-mono text-sm font-semibold text-blue-600">{returnItem.trackingId || '—'}</p>
                                        {getStatusBadge(returnItem.status)}
                                    </td>
                                    <td className="px-6 py-4">
                                        <p className="font-mono text-sm font-medium text-slate-900">{returnItem.orderNumber}</p>
                                        <p className="text-xs text-slate-500 mt-1">Item: {returnItem.id}</p>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-sm font-medium text-slate-900">{returnItem.quantity}x {returnItem.productName}</p>
                                                <p className="text-xs text-slate-500">Reason: {returnItem.returnReason}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        ...
                                        <p className="text-xs text-slate-500 mt-2">ID: {returnItem.id}</p>
                                        {returnItem.refundAmount > 0 && (
                                            <p className="text-sm font-semibold text-emerald-600 mt-1">₹{returnItem.refundAmount.toLocaleString()}</p>
                                        )}
                                    </td>
                                </tr>
                            ))}
                ...
                {filteredReturns.length === 0 && (
                    <div className="py-16 text-center">
                        <div className="mx-auto mb-4 h-32 w-32 rounded-full bg-slate-100 flex items-center justify-center"><RotateCcw className="h-12 w-12 text-slate-300" /></div>
                ...
                <div className="flex items-center justify-between border-t border-slate-200 bg-slate-50 px-6 py-4">
                    ...
                    <span className="text-sm text-slate-600">{filteredReturns.length} items</span>
                </div>
            </div>
        </div>
    );
