...
export default function SchoolProductManagementPage() {
    ...
    useEffect(() => {
        if (selectedCategory !== 'all' && allowedTypes && !allowedTypes.includes(selectedCategory)) {
    ...
    const addressLine = [school.address?.line1, school.address?.line2].filter(Boolean).join(', ');
    const fullAddress = [addressLine, school.city, school.state, school.postal_code || school.postalCode].filter(Boolean).join(', ');
    ...
    const filteredProducts = useMemo(() => {
        ...
        if (allowedTypes) {
            products = products.filter(
                (p) => allowedTypes.includes(p.product_type)
            );
        }
        ...
        if (selectedCategory !== 'all') {
            products = products.filter(
                (p) => p.product_type === selectedCategory
            );
        }
    ...
    const tabs = [
        { id: 'my_products', label: 'Listed Products', count: filteredProducts.length },
        { id: 'track_approvals', label: 'Track Approvals', count: pendingProducts.length },
    ...
    if (hasError) {
        return (
            <div className="flex flex-col items-center justify-center py-32">
                ...
                <p className="text-lg font-medium text-slate-700">Failed to load school</p>
                <p className="text-sm text-slate-500 mt-1 mb-6 max-w-sm text-center">{schoolDetailError}</p>
                <div className="flex gap-3">
                    <Button variant="outline" onClick={() => navigate('/dashboard/inventory/schools')}>
            ...
            {activeTab === 'my_products' && (
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Button onClick={() => navigate(`/dashboard/inventory/general/add?schoolId=${schoolId}`)}>
                ...
                <>
                                    ...
                                    {filteredProducts.map((product) => (
                                        ...
                                        >
                                            ...
                                            <div className="aspect-square bg-slate-100 flex items-center justify-center p-6 overflow-hidden">
                                                {product.primaryImage || product.image_url || (product.images && product.images[0]?.url) ? (
                                                    <img
                                                        src={product.primaryImage || product.image_url || product.images[0]?.url}
                                            ...
                                            <div className="p-4">
                                                ...
                                                {product.schoolInfo && (
                                                    <div className="flex items-center gap-2 mt-2">
                                                        <p className="text-xs text-blue-600 font-medium">Grade: {product.schoolInfo.grade}</p>
                                                        {product.schoolInfo.mandatory && (
                                                            <span className="text-xs px-1.5 py-0.5 rounded bg-amber-100 text-amber-700 font-medium">Mandatory</span>
                                                        )}
                                                    </div>
                                                )}
                                            ...
                                            {filteredProducts.map((product) => (
                                                <tr key={product.id} className="hover:bg-slate-50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 text-sm text-slate-700">
                                                        ...
                                                        {product.schoolInfo?.mandatory && (
                                                            <span className="ml-1.5 text-xs px-1.5 py-0.5 rounded bg-amber-100 text-amber-700 font-medium">Mandatory</span>
                                                        )}
                                                    </td>
                                                    <td className="px-6 py-4">
                                                        {product.is_active !== undefined ? (
                    ...
                    {filteredProducts.length === 0 && (
                        <div className="py-16 text-center">
                            ...
                            <Button
                                className="mt-4"
                                onClick={() => navigate(`/dashboard/inventory/general/add?schoolId=${schoolId}`)}
                            >
                                <Plus className="h-4 w-4" />
                                Add Product
                            </Button>
                ...
                <div className="space-y-4">
                    ...
                    {pendingProducts.length > 0 ? (
                        pendingProducts.map((product) => (
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="font-medium text-slate-900">{product.title || product.name}</p>
                                        <div className="flex items-center gap-3 mt-1">
                                            <p className="text-sm text-slate-500">SKU: {product.sku || '—'}</p>
                                            <span className="text-slate-300">·</span>
                                            <p className="text-sm text-slate-500">₹{product.base_price ?? '—'}</p>
                                            {product.schoolInfo?.grade && (
                                                <>
                                                    <span className="text-slate-300">·</span>
                                                    <p className="text-sm text-slate-500">Grade: {product.schoolInfo.grade}</p>
                                                </>
                                            )}
                                        </div>
                                        {product.created_at && (
                                            <p className="text-xs text-slate-400 mt-1 flex items-center gap-1">
                                                ...
                                                Submitted: {new Date(product.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                                            </p>
                                        )}
                                    </div>
                                </div>
                        ...
                        <div className="py-16 text-center">
                            ...
                            <p className="text-lg font-medium text-slate-600">All products are approved</p>
                            <p className="mt-2 text-sm text-slate-500">
            ...
            {activeTab === 'analytics' && (
                <div className="space-y-6">
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                        <h3 className="text-sm font-semibold text-slate-900 mb-4">School Information</h3>
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            <InfoItem label="Board" value={school.board} />
                            <InfoItem label="Type" value={school.type} className="capitalize" />
                            <InfoItem label="City" value={school.city} />
                            <InfoItem label="State" value={school.state} />
                            <InfoItem label="Postal Code" value={school.postal_code || school.postalCode} />
                            <InfoItem label="Status" value={school.is_active ? 'Active' : 'Inactive'} />
                            {contactPhone && <InfoItem label="Phone" value={contactPhone} />}
                            {contactEmail && <InfoItem label="Email" value={contactEmail} />}
                            {contactWebsite && <InfoItem label="Website" value={contactWebsite} isLink />}
                        </div>
                    </div>
                </div>
            )}
    ...
    return (
        <div>
            ...
            {isLink ? (
                ...
                <p className={cn('text-sm font-medium text-slate-900', className)}>{value}</p>
            )}
        </div>
    );
}