...
export default function RegisterPage() {
    ...
    const [emailVerified, setEmailVerified] = useState(resumeOnboarding ? true : false);
    ...
    useEffect(() => {
        const subscription = warehouseForm.watch((values) => {
            ...
            if (values.contactEmail !== undefined) whUpdateField('contactEmail', values.contactEmail);
            if (values.contactPhone !== undefined) whUpdateField('contactPhone', values.contactPhone);
            if (values.website !== undefined) whUpdateField('website', values.website);
            if (values.address) {
    ...
    const handleSendOtp = async () => {
        ...
        try {
            ...
            const result = await sendOtp(data.fullName, data.email, data.password, data.mobile);
    ...
    const handleEditEmail = () => {
        if (window.confirm("Editing your email will require re-verification. Continue?")) {
    ...
    const handleIdSigSubmit = async () => {
        // Validate ID fields
        const isValid = await businessForm.trigger(['businessCategory', 'ownerName', 'gstin', 'panNumber']);
        if (isValid) {
            if (!signature) {
            ...
            try {
                ...
                formData.append('displayName', businessData.businessName || businessData.ownerName); // Fallback if not set yet, though displayName is in next step?
                ...
                if (businessData.panNumber) formData.append('pan', businessData.panNumber);
                ...
                alert(error.response?.data?.message || 'Failed to save details. Please try again.');
            } finally {
                setIsLoading(false);
            }
        }
    ...
    const BreadcrumbHeader = () => {
        ...
        return (
            <div className="flex items-center gap-4 mb-6">
                ...
                <div className="flex items-center gap-2">
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center ${completedSteps.includes('email') ? 'bg-[#26A541]' :
                        currentStep === 'email' || currentStep === 'business' ? 'bg-[#2874F0]' : 'bg-gray-300'
                        }`}>
                        {completedSteps.includes('email') && completedSteps.includes('business') ? (
                            <CheckCircle className="w-3 h-3 text-white" />
                        ) : (
                            <span className="text-xs text-white font-medium">2</span>
                        )}
                    </div>
                    <span className={`text-sm font-medium ${completedSteps.includes('email') ? 'text-[#26A541]' :
                        (currentStep === 'email' || currentStep === 'business') ? 'text-[#212121]' : 'text-[#878787]'
                        }`}>
                        BUSINESS DETAILS
                    </span>
                </div>
            ...
            case 'mobile':
                return (
                    <div className="animate-fade-in relative">
                        ...
                        <h2 className="text-lg font-medium text-[#212121] mb-4">Register as Retailer</h2>

                        <form onSubmit={registerForm.handleSubmit(handleRegisterContinue)} className="space-y-5">
                            <Input
                                ...
                                rightElement={
                                    emailVerified ? (
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button type="button" onClick={handleEditEmail} className="text-[#2874F0] text-sm font-medium hover:underline">Edit</button>
                                        </div>
                                    ) : (
                                        <button type="button" onClick={handleSendOtp} className="text-[#2874F0] text-sm font-medium hover:underline" disabled={isLoading}>{isLoading ? 'Verifying...' : 'Verify'}</button>
                                    )
                                }
                            />
                            <Input
                            ...
                            <p className="text-xs text-[#878787]">
                                By continuing, I agree to Bukizz&apos;s <a href="#" className="text-[#2874F0] hover:underline">Terms of Use</a> & <a href="#" className="text-[#2874F0] hover:underline">Privacy Policy</a>
                            </p>
                            <Button type="submit" className="w-auto">Register & Continue <ArrowRight className="w-4 h-4" /></Button>
                        </form>
                    </div>
                );
            ...
            case 'email': // ID & Signature Verification (Step 2 in Sidebar)
                return (
                    <div className="animate-fade-in">
                        <h1 className="text-2xl font-semibold text-[#212121] mb-2">Business Details</h1>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="pt-4 border-t border-gray-200">
                                <h3 className="text-lg font-medium text-[#212121] mb-4">Add Your e-Signature</h3>
                                ...
                                {signature ? (
                                    <div className="relative border rounded-lg p-6 bg-gray-50 flex flex-col items-center justify-center group">
                                        {signature.type === 'drawn' ? (
                                            <img src={signature.data} alt="Signature" className="max-h-24" />
                                        ) : (
                                        ...
                                        <p className="text-xs text-gray-400 mt-2">Click X to remove and add new</p>
                                    </div>
                                ) : (
            ...
            case 'business': // Store & Pickup Details (Step 3 in Sidebar)
                return (
                    <div className="animate-fade-in">
                        ...
                        <h1 className="text-2xl font-semibold text-[#212121] mb-2">Store & Pickup Details</h1>
                        <p className="text-sm text-[#878787] mb-6">
                            Set up your warehouse / store details and pickup address for order fulfilment.
                        </p>
                        ...
                        {whIsCreated && (
                            <div className="mb-4 flex items-center gap-2 p-3 rounded-md bg-green-50 border border-green-100 text-green-800 text-sm">
                                ...
                                Warehouse already saved. You can update and re-submit, or continue to the next step.
                            </div>
                        )}

                        <form onSubmit={warehouseForm.handleSubmit(handleStoreSubmit)} className="space-y-6">
                            ...
                            <div className="space-y-4 pt-4 border-t border-gray-200">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <Input
                                        ...
                                        error={warehouseForm.formState.errors.address?.city?.message}
                                    />
                                    <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <Input
                                        ...
                                        error={warehouseForm.formState.errors.address?.postalCode?.message}
                                    />
                                    <Input
                                        label="Country"
                                        placeholder="India"
                                        {...warehouseForm.register('address.country')}
                                        error={warehouseForm.formState.errors.address?.country?.message}
                                    />
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <Input
                                        ...
                                        type="number"
                                        step="any"
                                        {...warehouseForm.register('address.lat', { valueAsNumber: true })}
                                    ...
                                    <Input
                                        ...
                                        type="number"
                                        step="any"
                                        {...warehouseForm.register('address.lng', { valueAsNumber: true })}
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                <Button type="submit" disabled={whIsSubmitting} variant="success">
                                    {whIsSubmitting ? 'Saving…' : (whIsCreated ? 'Update & Complete' : 'Save & Complete Registration')}
                        ...
                        <div className="p-8 sm:p-10 text-center">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-amber-700 leading-relaxed">
                                            Your application is now under review by our admin team. You will receive a
                                            <strong> confirmation email</strong> once your account has been approved.
                            ...
                            <div className="bg-slate-50 rounded-xl p-5 mb-8 text-left">
                                ...
                                <div className="space-y-3">
                                    {[
                                        { step: '1', text: 'Admin reviews your business details & documents' },
                                        { step: '2', text: 'You receive an approval email from Bukizz' },
                                        { step: '3', text: 'Log in and start managing your inventory & orders' },
                                    ].map(({ step, text }) => (
